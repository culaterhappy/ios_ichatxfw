...
- (void)connect {
    ...
    if(self.isCreated) {
        NSLog(@"JFRWebSocket...connect...isCreated is true...ignre and return...");
        return;
    }
    
    
    NSLog(@"JFRWebSocket...connect...isCreated is false...go on...");
...
- (void)disconnectStream:(NSError*)error {
    NSLog(@"JFRWebSocket...disconnectStream...");
        ...
        if (status == YES) {
            ...
            dispatch_async(self.queue,^{
                if([self.delegate respondsToSelector:@selector(websocketDidConnect:)]) {
                    NSLog(@"JFRWebSocket..._isConnected..will call websocketDidConnect...");
                    [weakSelf.delegate websocketDidConnect:self];
                }
                else{
                    NSLog(@"JFRWebSocket..._isConnected..will not all websocketDidConnect...");
                }
